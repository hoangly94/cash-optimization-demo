...
import { FETCH_HISTORY, SELECT_HISTORY_ROW, FETCH_HISTORY_DETAIL } from '~stores/category/region/constants';
...
export const Element = (props: Popup.Props) => {
  ...
  const historySelector = useSelector(state => state['region'].history);
  //create props
  ...
  return (
    <Popup.Element {...props}>
      ...
      <Block.Element {...actionsWrapperProps}>
        <Pagination.Element
          store={{
            totalSelectorKeys: ['region', 'history'],
            action: {
              type: FETCH_HISTORY,
            }
          }}
        ...
        <Block.Element>
          <Button.Element
            ...
            store={{
              isDisabledSelectorKeys: ['base', 'buttons', 'region', 'historyDetail'],
              action: {
                type: HANDLE_POPUP,
            ...
            onClick={() => dispatch({
                type: HANDLE_POPUP,
                keys: ['region', 'historyDetail', 'isShown'],
                value: true,
                popupType: 3,
            })}
          />
...
const handleRowClick = (dispatch) => (item) => (e) => {
  ...
  dispatch({ type: HANDLE_BUTTON, keys: ['region', 'historyDetail', 'isDisabled'], value: false });
}
