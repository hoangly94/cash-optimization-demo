...
import { REQUEST_CREATING, REQUEST_CREATING_CANCEL, CHANGE_CREATING_INPUT } from '~stores/category/region/constants';
...
export const Element = (props: Popup.Props) => {
  ...
  const creatingPopupSelector = useSelector(state => state['region'].creatingPopup);
  ...
  return (
    <Popup.Element {...props}>
      ...
      <Block.Element {...inputWrapperProps}>
        <Title.Element text='Mã vùng' {...inputTitleProps} />
        <Input.Element
          placeholder='Mã vùng'
          {...inputProps}
          store={{
            selectorKeys: ['region', 'creatingPopup', 'regionCode'],
...
const validateForm = (creatingPopupSelector) => {
  if (!creatingPopupSelector.regionCode)
    return false;
  return true;
}
