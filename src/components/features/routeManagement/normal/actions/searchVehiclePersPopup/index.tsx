...
import { HANDLE_DUALTABLE_MOVE, SELECT_COMBOX, REQUEST_QUERY, SELECT_DUALTABLE_CONTENT_ROW, SET_POPUP_TYPE, HANDLE_SPECIAL_DELETE, HANDLE_SPECIAL_ADD, REQUEST_EDITING_CANCEL, REQUEST_EDITING, CHANGE_EDITING_INPUT, REQUEST_UPDATE_CONTINUE, REQUEST_EDITING_SEACHVEHICLEPERS, REQUEST_SEACHVEHICLEPERS, REQUEST_SEACHVEHICLEPERS_CONTINUE, REQUEST_SEACHVEHICLEPERS_CANCEL, } from '~stores/routeManagement/normal/constants';
...
export const Element = (props: Popup.Props) => {
  ...
  const handleSubmitButtonClick = () => {
    ...
    if (isValidForm) {
      dispatch({ type: REQUEST_SEACHVEHICLEPERS });
      if (setIsShown)
        setIsShown(false)
    }
  }
  const handleContinueButtonClick = () => {
    const isValidForm = validateForm(dispatch, selector);
    if (isValidForm) {
      dispatch({ type: REQUEST_SEACHVEHICLEPERS_CONTINUE });
      if (setIsShown)
        setIsShown(false)
    }
  }
