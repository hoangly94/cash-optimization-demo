...
export const Element = (props: Popup.Props) => {
  ...
  const orgsCodeOptions = {
    $options: orgsList.map(item => ({ ...item, key: item.$children.value, onClick: handleOptionClick(dispatch, SELECT_ORGS_CODE_EDITING, item.$children) })),
  }
  const atmCdmStatusOptions = {
    $options: atmCdmStatusList.map(item => ({ ...item, key: item.$children.value, onClick: handleOptionClick(dispatch, SELECT_ATMCDM_STATUS_EDITING, item.$children) })),
  };
