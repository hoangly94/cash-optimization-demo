...
function* createDataSaga() {
    const state = yield select();
    const responseData = yield call(requestCreating, process.env.PATH + '/api/cashoptimization/createCategoryArea', state.area.creatingPopup);
...
function* editDataSaga() {
    yield put({ type: FETCH_DATA });
    const state = yield select();
    const responseData = yield call(requestEditing, process.env.PATH + '/api/cashoptimization/updateCategoryArea', state.area.selectedItem);
...
function getHistory(action, data) {
    ...
    const url = process.env.PATH + '/api/cashoptimization/historyCategoryAreaByCode';
    const postData = {
        data: {
            sort: sort,
            page: page,
...
function getData(filters, action) {
    ...
    const url = process.env.PATH + '/api/cashoptimization/findCategoryArea';
    const postData = {
        data: {
